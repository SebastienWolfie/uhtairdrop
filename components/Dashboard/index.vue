        ...
        <div class="container">
            <header>
              <div class="uht-mark">
                  ...
                  <img src="https://cdn.jsdelivr.net/gh/tabletophq/hosted-assets/uht-quest-jar-placeholder.png" alt="UHT Quest S2 logo" onerror="this.src='https://dummyimage.com/96x96/6a44ff/ffffff&text=UHT'"/>
                  <div>
            ...
            <section class="hero">
              <div class="hero-card">
                  <h1>Earn daily <span style="color:#fff; background:linear-gradient(90deg,#fff,#ffe9ff);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent">Health Points</span></h1>
                  <p class="lead">Earn daily Health points on the UHT reward cycle. To earn points, answer the daily quest questions covering quick tips, practical wellness habits, and medical knowledge checks. <strong>Points will be converted to UHT at the end of the Season 2 campaign.</strong></p>
                  ...
                  <span class="badge" title="Status"><small>Address: <strong>{{ auth.walletAddress?.slice(0,4) }}...{{ auth.walletAddress?.slice(auth.walletAddress?.length-4) }}</strong></small></span>
                  ...
                  <div class="stats">
                  <div class="stat"><div class="label">Points earned</div><div class="num" id="points">{{ auth?.points + referralPoints || 0 }}</div></div>
                  <!-- <div class="stat"><div class="label">Streak</div><div class="num" id="streak">0 days</div></div> -->
                  <div class="stat"><div class="label">Season</div><div class="num">S2</div></div>
                  </div>
              </div>
                ...
                <div id="qWrap">
                    ...
                    <div class="text-[16px]" v-else>
                        <div v-if="dailyQuestion?.id">
                            <p>The correct answer: {{ dailyQuestion?.options?.find(i => i.id == dailyQuestion?.answerID)?.text }}</p>
                            <p class="mt-1">Your answer: {{ dailyQuestion?.options?.find(i => i.id == auth?.lastAnswerID)?.text }}</p>
                        </div>  
                    </div>
            ...
            <div class="panel">
                <div class="feature"><span class="dot"></span><p><strong>Gamified health</strong> — daily tips, quick quizzes, and medical questions to grow your Health IQ.</p></div>
                <div class="feature" style="margin-top:10px"><span class="dot" style="background:#7ad7ff;box-shadow:0 0 0 4px rgba(122,215,255,.25)"></span><p><strong>Airdrop-ready</strong> — your points are a key criterion to receive the UHT airdrop at the end of Season 2.</p></div>
                <div class="feature" style="margin-top:10px"><span class="dot" style="background:#ffd166;box-shadow:0 0 0 4px rgba(255,209,102,.25)"></span><p><strong>Own your streak</strong> — show up daily to multiply rewards and climb tiers.</p></div>
            </div>
            </section>
...
<script setup>
    ...
    import { getDailyQuestion } from '../../api/surveytask'
    import { updateSurveyAnswer, updateUserHasClaimedPoints, updateUser, addWalletAddressLowerField } from '../../api/profile'
...
<style scoped>
  :root{
    ...
    --brand-2:#8358ff;        /* lighter accent */
    --ink:#0d0d12;            /* near-black text */
    --text:#f3f4f8;           /* light text */
    --glass:rgba(255,255,255,.08);
    --stroke:rgba(255,255,255,.18);
    --card:#ffffff;           /* plain white for light cards */
    --ok:#31d0aa;             /* success accent */
    --warn:#ffd166;           /* warm accent */
  }
  *{box-sizing:border-box}
  ...
  .brand-title{font-weight:800;font-size:clamp(24px,3vw,32px);letter-spacing:.4px}
  ...
  .hero{
    display:grid;grid-template-columns: 1.1fr .9fr;gap:28px;align-items:center;margin-top:8px;
  }
  @media (max-width: 900px){.hero{grid-template-columns:1fr}}
  .hero-card{
    background:var(--glass);border:1px solid var(--stroke);border-radius:22px;padding:28px;backdrop-filter: blur(8px);
    box-shadow:0 20px 50px rgba(0,0,0,.25);
  }
  h1{margin:0 0 10px;font-size:clamp(34px,6vw,56px);line-height:1.02;letter-spacing:-.02em}
  ...
  .btn{
    appearance:none;cursor:pointer;padding:12px 18px;border-radius:14px;font-weight:700;letter-spacing:.2px;
    transition:transform .08s ease, box-shadow .2s ease, background .2s ease;
  }
  .btn-primary{background:#fff;color:#3e2cff;box-shadow:0 10px 20px rgba(0,0,0,.20), 0 0 0 2px rgba(255,255,255,.15) inset}
  ...
  .stat{background:rgba(255,255,255,.09);border:1px solid var(--stroke);border-radius:16px;padding:16px}
  .stat .label{opacity:.8;font-size:12px}
  .stat .num{font-weight:800;font-size:28px}

  .panel{
    background:rgba(255,255,255,.06);border:1px solid var(--stroke);border-radius:18px;padding:22px
  }
  .split{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:18px}
  @media (max-width:900px){.split{grid-template-columns:1fr}}

  .question{
    background:#0f0f16; border:1px solid #2b2b3a; color:#eaeaf2; border-radius:16px;padding:18px
  }
  .question h3{margin:0 0 6px}
  .empty{opacity:.75}

  .feature{
    display:flex;gap:12px;align-items:flex-start
  }
  .feature .dot{width:12px;height:12px;border-radius:3px;background:var(--ok);box-shadow:0 0 0 4px rgba(49,208,170,.25)}
  .feature p{margin:0}

  footer{opacity:.8;margin:28px 0 10px;font-size:12px;text-align:center}

  /* Tiny flourish on the jar */
  .jar-wrap{display:flex;justify-content:center}
  .jar{
    width:min(400px, 88%);
    aspect-ratio:1/1; border-radius:24px; background:linear-gradient(180deg,#8d6bff22 0%, #ffffff10 100%);
    display:grid;place-items:center; padding:18px; border:1px solid var(--stroke);
  }
