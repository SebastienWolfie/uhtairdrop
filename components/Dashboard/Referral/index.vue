      ...
      <section class="hero">
        ...
        <div class="hero-card">
          ...
          <section v-if="sortedReferrals.length" class="mt-10">
            ...
            <p class="mb-4" v-if="showTimer">Time expires in <span class="text-red-300 font-bold text-[20px]" id="referral_claim_bid_timer"></span></p>
...
<script setup>
import { updateAddressProfile, getUser } from '../../../api/profile';
...
.badge { display:inline-flex; align-items:center; gap:8px; padding:6px 10px; border-radius:999px;
  background:rgba(255,255,255,.12); border:1px solid var(--stroke); backdrop-filter: blur(6px); }
...
h1 { margin:0 0 10px; font-size:clamp(34px,6vw,56px); line-height:1.02; }
...
.btn-primary { background:#fff; color:#3e2cff; box-shadow:0 10px 20px rgba(0,0,0,.20); }
.btn-primary:hover { transform:translateY(-1px); }

.jar-wrap { display:flex; justify-content:center; }
.jar { width:min(350px, 88%); aspect-ratio:1/1; border-radius:24px; 
